...
section = "Reference"
order = 27
summary = "`agentgraph install-skill` installs the bundled Graph skill into a user or project agent-skills directory."
...
The default target is `~/.agents/skills/graph`. A project target installs into
`./.agents/skills/graph`. `--claude` also creates the matching link under
