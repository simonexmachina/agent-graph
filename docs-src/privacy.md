...
- Remove configured RSS feeds with `agentgraph connector rss remove` or observed web URLs with `agentgraph connector web observe <url> --remove`. Removing an RSS feed also removes its feed Folder and edges; indexed articles follow normal retention.
