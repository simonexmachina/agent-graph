"""TLS compatibility handling for RSS feeds with incomplete certificate chains."""

from __future__ import annotations

import ssl
from urllib.parse import urlparse

_WOLFRAM_HOST = "writings.stephenwolfram.com"

# The site currently omits this public Let's Encrypt YR1 intermediate.
# It expires on 2028-09-02; remove this fallback once the origin sends it.
_LE_YR1_INTERMEDIATE = """-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"""


def feed_tls_verify(feed_url: str) -> ssl.SSLContext | bool:
    """Return strict TLS verification settings for a feed URL."""
    if urlparse(feed_url).hostname != _WOLFRAM_HOST:
        return True

    context = ssl.create_default_context()
    context.load_verify_locations(cadata=_LE_YR1_INTERMEDIATE)
    context.verify_flags |= ssl.VERIFY_X509_PARTIAL_CHAIN
    return context
